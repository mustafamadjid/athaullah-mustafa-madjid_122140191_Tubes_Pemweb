...
const PesananPage = () => {
  ...
  // UID Pembeli
  const uidPembeli = user?.uid || "";

  // URL Pesanan
  const url = `${import.meta.env.VITE_API_URL}/pesanan/pembeli/${uidPembeli}`;

  // Fetch pesanan Hooks
  const pesananFetch = useFetch(url);

  // Fetch pembeli
  const pembeliFetch = useFetch(
    `${import.meta.env.VITE_API_URL}/pembeli/profil/${uidPembeli}`
  );
  ...
  useEffect(() => {
    console.log(pembeliFetch.response);
